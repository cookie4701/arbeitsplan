...
$sdate = $helper->CalendarWeekStartDate($week, $helper->getUserStartYear($userid) );

$day = substr($sdate, 8, 2);
$month = substr($sdate,5,2);
$year = substr($sdate,0,4);

$ret = $helper->getWorkWeek($userid, $day, $month, $year);

echo "<p class=\"repName\">" . $helper->getDisplayName($userid) . ", Woche: $week </p>";
...
echo "<table>";
echo "<tr>";
echo "<td>Alte &Uuml;berstunden: </td><td>" . $ret['overhoursbeforeweek'] . "</td><td> Verbleibende Urlaubstage: </td><td>" . $ret['hollidaynow'] . "</td>";
echo "</tr>";

echo "<tr>";
echo "<td>Soll (diese Woche): </td><td>" . $workToDo . "</td><td>  </td>";
echo "</tr>";

$hoursWorkedWeek = (int) ($minutesDoneWeek / 60);
$minutesDoneWeek = $minutesDoneWeek - ($hoursWorkedWeek * 60);
echo "<tr>";
echo "<td>Geleistet (diese Woche):</td><td>$hoursWorkedWeek:$minutesDoneWeek</td> <td> </td> </td>";
echo "</tr>";


echo "<tr>";
echo "<td>Neue &Uuml;berstunden: </td><td>" . $ret['overhours'] . "</td><td>Verbleibende Feiertage: </td><td>" . $ret['vacationnow'] . "</td>";
echo "</tr>";
echo "</table>";
